...
/**
 * A bean factory that reflectively reads and injects an ObjectId into a target
 * bean instance.
 *
 */
public class DefaultBeanFactory<T> implements BeanFactory<T> {
    ...
    /**
     * attempts to inject a new {@link ObjectId} instance in the field annotated
     * with @Id
     * 
     * @param target
    ...
    protected ObjectId getIdValue(T target) throws ReflectiveOperationException {
        ...
        for (Method m : beanClass.getMethods()) {
            if (m.getName().equals("getId")) { return (ObjectId) m
                    .invoke(target); }
        }
     ...
     *             {@link org.springframework.data.annotation.Id}
     */
    protected void setIdValue(T target, ObjectId value)
